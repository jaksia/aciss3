...
export const eventDataValidator = v.pipe(
	v.object({
...
export const eventChangePasswordValidator = v.pipe(
	v.object({
		eventId: v.number(),
		_currentPassword: v.optional(v.string()),
		_password: v.pipe(
			v.string(),
			v.trim(),
			...
			v.check((pwd) => {
				...
				return pwd.length >= 6 && pwd.length <= 255;
			}, 'Password must be between 6 and 255 characters long')
		),
		_passwordConfirm: v.pipe(
			v.string(),
			v.trim(),
			v.transform((s) => s || null)
		)
	...
	v.forward(
		v.partialCheck(
			...
			(data) => data._password === data._passwordConfirm,
